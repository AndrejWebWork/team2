  ...
  return (
    <div className='consent-overlay fixed inset-0 z-[2500] flex items-center justify-center bg-slate-900/30 p-4 backdrop-blur-sm'>
      ...
      >
        ...
        <p className='mt-2.5 text-sm font-semibold text-slate-900'>{t('cookie.title')}</p>
        <p className='mt-1 text-xs leading-relaxed text-slate-600'>{t('cookie.body')}</p>
        <button
          type='button'
          onClick={() => navigate('/legal?tab=privacy')}
          className='mt-2 text-xs font-medium text-emerald-600 hover:text-emerald-700 hover:underline'
        >
          {t('cookie.learn')}
        </button>
        <div className='mt-3.5 flex flex-col gap-2'>
          <Button size='sm' onClick={() => decide('all')} className='w-full'>{t('cookie.accept')}</Button>
          <Button size='sm' variant='outline' onClick={() => decide('necessary')} className='w-full'>{t('cookie.necessary')}</Button>
        </div>
      </div>
    </div>
  )
