...
export function ContainersPage() {
  ...
  function resetIssue(id) {
    ...
    setContainers((prev) =>
      prev.map((c) => {
        if (c.id !== id) return c
        // +2 поени при решено (вкупно 3) — само за регистрирани корисници.
  ...
  return (
    <div className='space-y-7'>
            ...
            {activeContainerIssues.map((c) => (
              <article key={c.id} className='group rounded-2xl border border-amber-100 bg-white p-5 shadow-sm transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5'>
                ...
                <div className='mt-3 space-y-1'>
                  ...
                  {c.fill != null && <p className='text-sm text-slate-600'>{t('cont.fill')}: {c.fill}%</p>}
      ...
      <section className='space-y-4'>
        <div>
          <h2 className='text-xl font-bold text-slate-900'>{t('cont.resolvedTitle')}</h2>
          <p className='mt-0.5 text-sm text-slate-500'>{t('cont.resolvedSubtitle')}</p>
        </div>
        {resolvedContainerReports.length === 0 ? (
          <EmptyState title={t('cont.noResolved')} description={t('cont.resolvedSubtitle')} />
        ) : (
          <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
            {resolvedContainerReports.map((c) => (
      ...
      <section className='space-y-4'>
        <h2 className='text-xl font-bold text-slate-900'>{t('cont.allContainers')}</h2>
        <div className='grid gap-3 sm:grid-cols-2 lg:grid-cols-3'>
          {containers.map((c) => {
            const issueMeta = {
              full:   { label: t('cont.full'), color: 'bg-amber-100 text-amber-700' },
              smell:  { label: t('cont.smell'), color: 'bg-rose-100 text-rose-700' },
              broken: { label: t('cont.broken'), color: 'bg-red-100 text-red-700' },
              none:   { label: t('cont.noProblem'), color: 'bg-emerald-100 text-emerald-700' },
            }
            const issue = issueMeta[c.issue] || issueMeta.none
            const fillColor = c.fill >= 80 ? 'bg-rose-500' : c.fill >= 50 ? 'bg-amber-400' : 'bg-emerald-500'
            return (
              <div key={c.id} className='rounded-2xl border border-slate-200 bg-white p-4 shadow-sm'>
                <div className='flex items-start justify-between gap-2'>
                  <p className='font-semibold text-slate-900'>{c.area}</p>
                  <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold ${issue.color}`}>{issue.label}</span>
                </div>
                <span className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${kindColor[c.kind] || 'bg-slate-100 text-slate-700'}`}>
                  {containerKindLabel(c.kind)}
                </span>
                {/* Пополнетоста се прикажува само ако е реално внесена (не измислена). */}
                {c.fill != null && (
                  <div className='mt-3'>
                    <div className='mb-1 flex items-center justify-between text-xs text-slate-500'>
                      <span>{t('cont.fill')}</span>
                      <span className='font-semibold text-slate-700'>{c.fill}%</span>
                    </div>
                    <div className='h-2 w-full overflow-hidden rounded-full bg-slate-100'>
                      <div className={`h-full rounded-full transition-all ${fillColor}`} style={{ width: `${c.fill}%` }} />
                    </div>
                  </div>
                )}
